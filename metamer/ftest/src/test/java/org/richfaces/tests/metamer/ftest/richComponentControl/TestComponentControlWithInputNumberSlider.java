 ...
 */
public class TestComponentControlWithInputNumberSlider extends AbstractComponentControlTest {

    private static final String[] OPERATIONS = new String[]{ "setValue", "increase", "decrease",/* "getValue" */ };

    @FindBy(className = "controlledComponent")
    private RichFacesInputNumberSlider input;
    ...
    protected void handleOperation(String operation) {
        assertEquals(input.advanced().getInput().getIntValue(), 2);
        getButtonWithOperation(operation).click();
        if ("setValue".equals(operation)) {
            assertEquals(input.advanced().getInput().getIntValue(), 10);
        } else if ("increase".equals(operation)) {
            assertEquals(input.advanced().getInput().getIntValue(), 3);
        } else if ("decrease".equals(operation)) {
            assertEquals(input.advanced().getInput().getIntValue(), 1);
        }
    }
