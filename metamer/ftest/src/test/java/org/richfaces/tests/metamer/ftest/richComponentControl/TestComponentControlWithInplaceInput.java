 ...
 */
public class TestComponentControlWithInplaceInput extends AbstractComponentControlTest {

    private static final String[] OPERATIONS = new String[]{ "setValue", "save", "cancel",/* "getValue", "isEditState", "isValueChanged", "getInput" */ };
    ...
    protected void handleOperation(String operation) {
        if ("setValue".equals(operation)) {
            ...
            assertEquals(input.getTextInput().getStringValue(), "new value");
        } else if ("save".equals(operation)) {
            String val = "This will be saved";
            input.type(val);
            mouseoverButton(getButtonWithOperation(operation));
            assertTrue(input.getTextInput().getStringValue().endsWith(val));
        } else if ("cancel".equals(operation)) {
            String val = "This will not be saved";
            input.type(val);
            mouseoverButton(getButtonWithOperation(operation));
            assertFalse(input.getTextInput().getStringValue().endsWith(val));
        }
    }
